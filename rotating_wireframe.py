...
# New configuration for rotation
MAX_ROTATION_SPEED = 10.0  # Maximum degrees per frame
MIN_ROTATION_SPEED = 1.0  # Minimum degrees per frame
...
persistent_settings = {
    ...
    'scale': 1.0,
    'aspect': 'auto',
    'offset': [0, 0, 0],  # track pan offsets
    'current_rotation': 0.0  # track current rotation angle
}
...
def get_dominant_frequency(fft_data, freqs):
    """Calculate the dominant frequency from FFT data"""
...
def frequency_to_rotation_speed(freq, freq_limit_low, freq_limit_high):
    """Convert frequency to rotation speed"""
    if freq == 0:
        return MIN_ROTATION_SPEED
        
    # Normalize frequency to 0-1 range
    normalized_freq = (freq - freq_limit_low) / (freq_limit_high - freq_limit_low)
    normalized_freq = np.clip(normalized_freq, 0, 1)
    
    # Convert to rotation speed
    speed = MIN_ROTATION_SPEED + (MAX_ROTATION_SPEED - MIN_ROTATION_SPEED) * normalized_freq
    return speed
...
def update(frame):
    ...
    try:
        ...
        # Get dominant frequency and calculate rotation speed
        dominant_freq = get_dominant_frequency(fft_data, filtered_freqs)
        rotation_speed = frequency_to_rotation_speed(dominant_freq, FREQ_LIMIT_LOW, FREQ_LIMIT_HIGH)
        
        # Update rotation angle
        persistent_settings['current_rotation'] += rotation_speed
        if persistent_settings['current_rotation'] >= 360:
            persistent_settings['current_rotation'] -= 360
        ...
        # Set view with automatic rotation
        ax.view_init(current_elev, persistent_settings['current_rotation'])
        ...
        # Maintain scale and aspect ratio
